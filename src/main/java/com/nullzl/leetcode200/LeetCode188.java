...
public class LeetCode188 {
    ...
    public int maxProfit1(int k, int[] prices) {
        ...
        if(0 == k || null == prices || 0 == prices.length)
            return 0;
        ...
        for(int z = 2 ;  z <= k ; z++){
            for(int x = len - 2 ; x >= 0  ; x--){
                for(int y = x + 1 ; y < len ; y++){
                    int profit = Math.max(p[x][y-1][z],p[x][y][1]);
                    for(int i = y-1; i > x ; i--){
                        int temp = p[i][y][1] + p[x][i-1][z-1];
                        if(temp > profit)
                            profit = temp;
                    }
                    p[x][y][z] = profit;
    ...
    /**
     * 状态转换
     * 对于每一天，有两种状态，持有股票，没有持有股票
     * 对于每一天，也需要记录，最大已购买股票的次数，因为有购买次数限制
     * 对于每一天，可以有三种操作，买入股票，保持不变，卖出股票
     * 所以定义数组p[i][j][k],k=0表示第i天，已购买j次股票，当前没有持有股票的最大收益
     * k=1，表示第i天，已购买j次股票，当前持有股票的最大收益
     * 递推公式：
     * p[i][j][0] = max{p[i-1][j][0],p[i-1][j][1] + prices[i]}  前一天没有持有股票或者前一天持有，今天卖出
     * p[i][j][1] = max{p[i-1][j][1],p[i-1][j-1][0] - prices[i]} 前一天已经持有，或者前一天没有持有，今天买入
     *
     */
    public int maxProfit(int k, int[] prices) {
        ...
        if(k > prices.length / 2){
            ...
            for(int i = 1;i < prices.length ; i++){
                if(prices[i] > prices[i-1] && !buyed){
                    profit -= prices[i-1];
                    buyed = true;
                }else if(prices[i] < prices[i-1] && buyed){
                    profit += prices[i-1];
                    buyed = false;
                }
            }
        ...
        for(int i = 1 ; i < prices.length ; i++){
            ...
            for(int j = 0 ; j < k ;j++){
                int temp1 = p[0][j];
                int temp2 = p[1][j];
                p[0][j] = Math.max(temp1,temp2 + prices[i]);
                p[1][j] = Math.max(temp2,preVal - prices[i]);
                preVal = temp1;
            }
        }
        return p[0][k-1];
